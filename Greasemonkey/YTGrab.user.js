    ...
    GM_xmlhttpRequest({method: "GET", url: unsafeWindow.ytplayer.config.assets.js.replace(/^\/\//, "https://"), onload: function (t) {
        ...
        var
            uriencToMap = (function (s) {
                var n = {}, a = s.split("&");
                for (var idy = 0; idy < a.length; idy++) {
                    var c = a[idy].split("=");
            ...
            fmtrgx = /^[-\w+]+\/(?:x-)?([-\w+]+)/,
            fmt_map = {}, idx, n, a, qual, fmt;

        var fmt_list = uwyca.fmt_list.split(",");
        ...
        var uefmss = uwyca.url_encoded_fmt_stream_map.split(",");
        ...
        var dashlist = uwyca.adaptive_fmts;
